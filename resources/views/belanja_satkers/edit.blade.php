...
@section('title', 'Edit Penyerapan Anggaran')
@section('page_title', 'Edit Penyerapan Anggaran')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        ...
        <h3 class="text-3xl font-black text-white tracking-tighter uppercase">Edit Penyerapan Anggaran</h3>
        <p class="text-slate-500 text-sm mt-1">Perbarui data realisasi penyerapan anggaran satker.</p>
    </div>

    <form action="{{ route('belanja-satker.update', $belanjaSatker) }}" method="POST" enctype="multipart/form-data" class="bg-[#031121] border border-[#D2A039]/20 rounded-[1.5rem] p-6 sm:p-8 shadow-xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="cabang_id" id="cabang_id" required class="w-full bg-[#111827] border border-slate-700 text-white rounded-xl focus:ring-[#D2A039]/20 focus:border-[#D2A039] p-3 transition-all">
                    ...
                    @foreach($cabangs as $cabang)
                        <option value="{{ $cabang->id }}" {{ old('cabang_id', $belanjaSatker->cabang_id) == $cabang->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $belanjaSatker->tahun) }}" required min="2020" class="w-full bg-[#111827] border border-slate-700 text-white rounded-xl focus:ring-[#D2A039]/20 focus:border-[#D2A039] p-3 transition-all">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="total" id="total" value="{{ old('total', $belanjaSatker->total) }}" required min="0" class="w-full bg-[#111827] border border-slate-700 text-white rounded-xl focus:ring-[#D2A039]/20 focus:border-[#D2A039] block pl-12 p-3 transition-all" placeholder="Contoh: 15000000">
                </div>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3" class="w-full bg-[#111827] border border-slate-700 text-white rounded-xl focus:ring-[#D2A039]/20 focus:border-[#D2A039] p-3 transition-all" placeholder="Tuliskan keterangan detail jika ada...">{{ old('keterangan', $belanjaSatker->keterangan) }}</textarea>
                @error('keterangan') <p class="mt-1.5 text-xs font-medium text-rose-500">{{ $message }}</p> @enderror
            </div>

            <!-- Upload Dokumen -->
            <div>
                <label for="dokumen" class="block text-xs font-bold text-slate-300 uppercase tracking-widest mb-2">Upload Dokumen Anggaran Baru (Opsional)</label>
                ...
                @if($belanjaSatker->dokumen_path)
                    <p class="text-xs text-[#D2A039] mt-2">
                        <i data-lucide="check-circle" class="w-3 h-3 inline mr-1"></i>
                        Dokumen saat ini sudah ada. Upload baru hanya jika ingin mengganti dokumen lama.
                    </p>
                @else
