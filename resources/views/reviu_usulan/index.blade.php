...
@section('title', 'Reviu Usulan Risiko Baru')
@section('page_title', 'Reviu Usulan Risiko')
...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
    <div class="relative group w-full max-w-md">
        ...
        <input type="text" id="searchInput" onkeyup="filterTable()" class="w-full bg-[#111827] border border-slate-800 text-white text-sm rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 block pl-14 p-4 transition-all" placeholder="Cari data reviu usulan...">
    </div>

    <a href="{{ route('reviu-usulan.create') }}" class="px-6 py-3 bg-blue-500 text-white font-bold rounded-2xl flex items-center hover:bg-blue-600 transition-all duration-300 shadow-xl shadow-blue-500/30 active:scale-95 shrink-0">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-800/40 border-b border-slate-800/60">
                    ...
                    <th rowspan="2" class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap border-r border-slate-800/60">Usulan Pernyataan Risiko</th>
                    <th rowspan="2" class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap border-r border-slate-800/60">Unit Pemilik Risiko Pengusul</th>
                    <th colspan="2" class="px-6 py-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-center border-b border-r border-slate-800/60">Status Reviu</th>
                    <th rowspan="2" class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap border-r border-slate-800/60">Alasan Jika Ditolak</th>
                ...
                <tr class="bg-slate-800/20 border-b border-slate-800/60">
                    <th class="px-4 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest text-center border-r border-slate-800/60">Diterima</th>
                    <th class="px-4 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest text-center border-r border-slate-800/60">Ditolak</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-800/40">
                @forelse($revius as $index => $r)
                <tr class="hover:bg-slate-800/30 transition-all group">
                    ...
                    <td class="px-6 py-4 border-r border-slate-800/60">
                        <span class="font-bold text-white tracking-tight text-sm block min-w-[250px]">
                            {{ $r->usulan_pernyataan_risiko }}
                        </span>
                        <span class="text-[10px] text-slate-500 mt-1 block">Kode: {{ $r->resiko->kode }}</span>
                    </td>
                    <td class="px-6 py-4 text-sm text-slate-400 border-r border-slate-800/60">{{ $r->unit_pemilik_pengusul }}</td>
                    ...
                    <td class="px-4 py-4 text-center border-r border-slate-800/60">
                        @if($r->status === 'Diterima')
                            <div class="flex flex-col items-center">
                                ...
                                <span class="text-[9px] text-emerald-400/60 mt-1 uppercase font-bold">{{ $r->alasan_diterima }}</span>
                            </div>
                        @else
                    ...
                    <td class="px-4 py-4 text-center border-r border-slate-800/60">
                        @if($r->status === 'Ditolak')
                            <div class="flex flex-col items-center">
                                ...
                                <span class="text-[9px] text-rose-400/60 mt-1 uppercase font-bold">Ditolak</span>
                            </div>
                        @else
                            <span class="text-slate-700">-</span>
                        @endif
                    ...
                    <td class="px-6 py-4 text-sm text-slate-400 border-r border-slate-800/60 italic">
                        {{ $r->status === 'Ditolak' ? $r->alasan_ditolak : '-' }}
                    </td>

                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end space-x-1">
                            <a href="{{ route('reviu-usulan.show', $r) }}" class="p-2 text-slate-500 hover:text-indigo-400 hover:bg-indigo-400/10 rounded-xl transition-all">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </a>
                            <a href="{{ route('reviu-usulan.edit', $r) }}" class="p-2 text-slate-500 hover:text-blue-400 hover:bg-blue-400/10 rounded-xl transition-all">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('reviu-usulan.destroy', $r) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-8 py-10 text-center text-slate-500 text-sm italic">Belum ada data reviu usulan risiko baru.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
