...
@section('page_title', 'Edit Analisis Risiko')
...
<div class="w-full bg-[#111827] border border-slate-800 rounded-[2.5rem] p-10 shadow-2xl">
    <div class="mb-10 relative z-10">
        <div class="flex items-center gap-4 mb-6">
            <div class="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 shrink-0">
                <i data-lucide="edit-3" class="w-6 h-6 md:w-7 md:h-7"></i>
            </div>
            <div>
                <h3 class="text-2xl font-black text-white tracking-tight">Edit Analisis Risiko</h3>
                <p class="text-slate-500 text-sm mt-1">Perbarui data analisis risiko yang telah diidentifikasi.</p>
            </div>
        </div>
    ...
    <form action="{{ route('analisis-risiko.update', $analisis_risiko) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="identifikasi_risiko_id" id="identifikasi_risiko_id" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            @foreach($identifikasi_risikos as $risiko)
                                <option value="{{ $risiko->id }}" data-pernyataan="{{ $risiko->pernyataan_risiko }}" {{ $analisis_risiko->identifikasi_risiko_id == $risiko->id ? 'selected' : '' }}>{{ $risiko->kode_risiko }} - {{ Str::limit($risiko->pernyataan_risiko, 50) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Pernyataan Risiko</label>
                        <div id="pernyataan_preview" class="w-full px-5 py-4 bg-slate-900/50 rounded-2xl border border-slate-800 text-slate-400 text-sm min-h-[58px] flex items-center">
                            {{ $analisis_risiko->identifikasiRisiko->pernyataan_risiko ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="frekuensi" id="frekuensi" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            ...
                            <option value="1" {{ $analisis_risiko->frekuensi == '1' ? 'selected' : '' }}>1 - Hampir tidak terjadi</option>
                            <option value="2" {{ $analisis_risiko->frekuensi == '2' ? 'selected' : '' }}>2 - Jarang terjadi</option>
                            <option value="3" {{ $analisis_risiko->frekuensi == '3' ? 'selected' : '' }}>3 - Kadang terjadi</option>
                            <option value="4" {{ $analisis_risiko->frekuensi == '4' ? 'selected' : '' }}>4 - Sering terjadi</option>
                            <option value="5" {{ $analisis_risiko->frekuensi == '5' ? 'selected' : '' }}>5 - Hampir pasti terjadi</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="dampak" id="dampak" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            ...
                            <option value="1" {{ $analisis_risiko->dampak == '1' ? 'selected' : '' }}>1 - Tidak Signifikan</option>
                            <option value="2" {{ $analisis_risiko->dampak == '2' ? 'selected' : '' }}>2 - Minor</option>
                            <option value="3" {{ $analisis_risiko->dampak == '3' ? 'selected' : '' }}>3 - Moderat</option>
                            <option value="4" {{ $analisis_risiko->dampak == '4' ? 'selected' : '' }}>4 - Signifikan</option>
                            <option value="5" {{ $analisis_risiko->dampak == '5' ? 'selected' : '' }}>5 - Sangat Signifikan</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Level Risiko (Melekat)</label>
                        <input type="text" name="level_risiko" id="level_risiko" value="{{ old('level_risiko', $analisis_risiko->level_risiko) }}" readonly class="w-full px-5 py-4 bg-slate-800/30 rounded-2xl border border-slate-700 text-white focus:ring-0 outline-none font-bold" placeholder="Otomatis terisi...">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="ada_belum_ada" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            <option value="Ada" {{ $analisis_risiko->ada_belum_ada == 'Ada' ? 'selected' : '' }}>Ada</option>
                            <option value="Belum Ada" {{ $analisis_risiko->ada_belum_ada == 'Belum Ada' ? 'selected' : '' }}>Belum Ada</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="memadai_belum_memadai" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            <option value="Memadai" {{ $analisis_risiko->memadai_belum_memadai == 'Memadai' ? 'selected' : '' }}>Memadai</option>
                            <option value="Belum Memadai" {{ $analisis_risiko->memadai_belum_memadai == 'Belum Memadai' ? 'selected' : '' }}>Belum Memadai</option>
                        </select>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Uraian Pengendalian</label>
                        <textarea name="uraian_pengendalian" rows="3" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none resize-none" placeholder="Masukkan uraian pengendalian...">{{ old('uraian_pengendalian', $analisis_risiko->uraian_pengendalian) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="skor_probabilitas_residu" id="skor_probabilitas_residu" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            <option value="" disabled hidden>-- Pilih Probabilitas --</option>
                            <option value="1" {{ $analisis_risiko->skor_probabilitas_residu == '1' ? 'selected' : '' }}>1 - Hampir tidak terjadi</option>
                            <option value="2" {{ $analisis_risiko->skor_probabilitas_residu == '2' ? 'selected' : '' }}>2 - Jarang terjadi</option>
                            <option value="3" {{ $analisis_risiko->skor_probabilitas_residu == '3' ? 'selected' : '' }}>3 - Kadang terjadi</option>
                            <option value="4" {{ $analisis_risiko->skor_probabilitas_residu == '4' ? 'selected' : '' }}>4 - Sering terjadi</option>
                            <option value="5" {{ $analisis_risiko->skor_probabilitas_residu == '5' ? 'selected' : '' }}>5 - Hampir pasti terjadi</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="skor_dampak_residu" id="skor_dampak_residu" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            <option value="" disabled hidden>-- Pilih Dampak --</option>
                            <option value="1" {{ $analisis_risiko->skor_dampak_residu == '1' ? 'selected' : '' }}>1 - Tidak Signifikan</option>
                            <option value="2" {{ $analisis_risiko->skor_dampak_residu == '2' ? 'selected' : '' }}>2 - Minor</option>
                            <option value="3" {{ $analisis_risiko->skor_dampak_residu == '3' ? 'selected' : '' }}>3 - Moderat</option>
                            <option value="4" {{ $analisis_risiko->skor_dampak_residu == '4' ? 'selected' : '' }}>4 - Signifikan</option>
                            <option value="5" {{ $analisis_risiko->skor_dampak_residu == '5' ? 'selected' : '' }}>5 - Sangat Signifikan</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Level Risiko (Residu)</label>
                        <input type="text" name="level_risiko_residu" id="level_risiko_residu" value="{{ old('level_risiko_residu', $analisis_risiko->level_risiko_residu) }}" readonly class="w-full px-5 py-4 bg-slate-800/30 rounded-2xl border border-slate-700 text-white focus:ring-0 outline-none font-bold" placeholder="Otomatis terisi...">
                    </div>
                </div>
            </div>
...
calculateRisk('frekuensi', 'dampak', 'level_risiko');
