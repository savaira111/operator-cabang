    ...
    <div class="w-full bg-[#031121]/50 backdrop-blur-xl border border-[#D2A039]/20 rounded-3xl md:rounded-[2.5rem] p-4 md:p-10 shadow-2xl overflow-hidden relative group">
        ...
        <form action="{{ route('analisis-risiko.store') }}" method="POST" class="relative z-10">
            ...
            <div class="space-y-8 md:space-y-12">
                ...
                <div class="p-0 md:p-0 bg-transparent md:bg-transparent rounded-none md:rounded-none border-none md:border-none space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                        <div class="space-y-1.5 md:space-y-2">
                            ...
                            <div class="relative group/select">
                                <select name="identifikasi_id" id="identifikasi_id" required class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/60 rounded-xl md:rounded-2xl border border-slate-800 text-white focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all outline-none appearance-none cursor-pointer text-xs md:text-sm">
                        ...
                        <div class="space-y-1.5 md:space-y-2">
                            <label class="block text-[10px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Lokasi / Unit</label>
                            <input type="text" id="lokasi" readonly class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/40 rounded-xl md:rounded-2xl border border-slate-800/50 text-slate-400 outline-none text-xs md:text-sm" placeholder="-">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5 md:space-y-2">
                        ...
                        <textarea id="pernyataan_risiko" readonly rows="2" class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/40 rounded-xl md:rounded-2xl border border-slate-800/50 text-slate-400 outline-none resize-none text-xs md:text-sm leading-relaxed" placeholder="-"></textarea>
                    </div>
                </div>

                <!-- SECTION 2: RISIKO MELEKAT -->
                <div class="p-4 md:p-0 bg-white/5 md:bg-transparent rounded-2xl md:rounded-none border border-white/5 md:border-none space-y-6">
                    <div class="flex items-center gap-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8">
                        <div class="md:col-span-4 space-y-1.5 md:space-y-2">
                            ...
                            <input type="number" name="melekat_kemungkinan" id="melekat_kemungkinan" min="1" max="5" required class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/60 rounded-xl md:rounded-2xl border border-slate-800 text-white focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all outline-none text-xs md:text-sm" placeholder="1-5">
                        </div>
                        <div class="md:col-span-4 space-y-1.5 md:space-y-2">
                            <label class="block text-[10px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Dampak (1-5)</label>
                            <input type="number" name="melekat_dampak" id="melekat_dampak" min="1" max="5" required class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/60 rounded-xl md:rounded-2xl border border-slate-800 text-white focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all outline-none text-xs md:text-sm" placeholder="1-5">
                        </div>
                        <div class="md:col-span-4">
                            <div id="melekat_result_container" class="h-full flex flex-col justify-center p-4 md:p-6 bg-slate-900/40 rounded-xl md:rounded-2xl border border-slate-800/50 transition-all">
                                <p class="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1">Besaran Risiko</p>
                                <div class="flex items-end gap-2">
                                    <span id="melekat_skor" class="text-2xl md:text-4xl font-black text-white leading-none">0</span>
                                    <span id="melekat_level" class="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Low</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-4 md:p-0 bg-white/5 md:bg-transparent rounded-2xl md:rounded-none border border-white/5 md:border-none space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5 md:space-y-2">
                            ...
                            <textarea name="pengendalian_uraian" rows="2" class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/60 rounded-xl md:rounded-2xl border border-slate-800 text-white outline-none focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all resize-none text-xs md:text-sm leading-relaxed" placeholder="Jelaskan pengendalian yang telah diimplementasikan..."></textarea>
                        </div>
                        <div class="space-y-1.5 md:space-y-2">
                            <label class="block text-[10px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Apakah Sudah Memadai?</label>
                            <div class="relative group/select">
                                <select name="pengendalian_memadai" required class="w-full px-4 py-3 md:px-5 md:py-4 bg-slate-900/60 rounded-xl md:rounded-2xl border border-slate-800 text-white outline-none focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all appearance-none cursor-pointer text-xs md:text-sm">
                                    <option value="Ya">Ya, Sudah Memadai</option>
                                    <option value="Tidak">Tidak, Belum Memadai</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-5 flex items-center pointer-events-none text-slate-500 group-hover/select:text-[#D2A039]">
                                    <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                </div>
                            </div>
                        </div>
                ...
                <!-- SECTION 4: RISIKO RESIDU -->
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group/select">
                                <select name="skor_dampak_residu" id="skor_dampak_residu" class="w-full px-5 py-4 bg-slate-900/60 rounded-2xl border border-slate-800 text-white focus:ring-4 focus:ring-[#D2A039]/10 focus:border-[#D2A039]/50 transition-all outline-none cursor-pointer appearance-none">
                        ...
                        <div class="space-y-2 sm:col-span-2 md:col-span-1">
                            <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Level Risiko Residu</label>
                            <div class="relative">
                                <input type="text" name="level_risiko_residu" id="level_risiko_residu" readonly class="w-full px-5 py-4 bg-slate-900/40 rounded-2xl border border-slate-800 text-white focus:ring-0 outline-none font-bold placeholder:text-slate-700" placeholder="Otomatis terisi...">
                ...
                <div class="pt-10 flex flex-col sm:flex-row items-center gap-4 border-t border-[#D2A039]/10 md:pb-0 pb-20">
                    <button type="submit" class="w-full sm:w-auto px-10 py-4 bg-gradient-to-r from-[#D2A039] to-[#f9d77e] hover:shadow-[0_0_25px_rgba(210,160,57,0.4)] text-[#061B30] font-black rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2">
                        <i data-lucide="save" class="w-5 h-5"></i>
                        <span>Simpan Analisis Risiko</span>
                    </button>
                    <a href="{{ route('analisis-risiko.index') }}" class="w-full sm:w-auto px-10 py-4 bg-transparent hover:bg-white/5 text-slate-400 font-bold rounded-2xl transition-all text-center">
                        Batal
                    </a>
                </div>
