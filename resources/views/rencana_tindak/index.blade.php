...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
    <div class="relative group w-full max-w-md">
        ...
        <input type="text" id="searchInput" onkeyup="filterTable()" class="w-full bg-[#111827] border border-slate-800 text-white text-sm rounded-2xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 block pl-14 p-4 transition-all" placeholder="Cari rencana tindak...">
    </div>

    <a href="{{ route('rencana-tindak.create') }}" class="px-6 py-3 bg-blue-500 text-white font-bold rounded-2xl flex items-center hover:bg-blue-600 transition-all duration-300 shadow-xl shadow-blue-500/30 active:scale-95 shrink-0">
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-800/40 border-b border-slate-800/60">
                    ...
                    <th class="px-8 py-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Kode WP</th>
                    <th class="px-8 py-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Rencana Tindak</th>
                    <th class="px-8 py-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Target Waktu</th>
                    <th class="px-8 py-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">PIC</th>
                ...
                <tr class="hover:bg-slate-800/30 transition-all group">
                    ...
                    <td class="px-8 py-5">
                        <span class="font-bold text-white tracking-tight group-hover:text-rose-400 transition-colors text-sm truncate max-w-[200px] block">
                            {{ $rencana->rencana_tindak ?? '-' }}
                        </span>
                    </td>
                    <td class="px-8 py-5 text-sm text-slate-400 font-bold tracking-tight">
                        {{ $rencana->waktu_pelaksanaan }}
                    </td>
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex items-center justify-end space-x-2 transition-all">
                            <a href="{{ route('rencana-tindak.show', $rencana) }}" class="p-2.5 text-slate-500 hover:text-indigo-400 hover:bg-indigo-400/10 rounded-2xl transition-all">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </a>
                            <a href="{{ route('rencana-tindak.edit', $rencana) }}" class="p-2.5 text-slate-500 hover:text-blue-400 hover:bg-blue-400/10 rounded-2xl transition-all">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('rencana-tindak.destroy', $rencana) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
