    ...
    <form action="{{ route('pemantauan-peristiwa.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    <div class="border border-slate-700/50 p-6 rounded-2xl bg-slate-800/20 mb-6">
                        ...
                        <div class="relative group">
                            <select name="rows[0][pemantauan_kegiatan_id]" onchange="updatePemantauanInfo(this)" required class="pemantauan_kegiatan_id w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none cursor-pointer appearance-none">
                    ...
                    <div class="preview-container hidden grid grid-cols-1 md:grid-cols-2 gap-6 opacity-80">
                        <div>
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Kode</label>
                            <p class="ro_kode text-sm font-bold text-emerald-400 bg-slate-900/50 p-4 rounded-xl border border-slate-800"></p>
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Kode Penyebab</label>
                            <p class="ro_kodepenyebab text-sm font-bold text-emerald-400 bg-slate-900/50 p-4 rounded-xl border border-slate-800"></p>
                        </div>
                    ...
                    <div class="border border-blue-500/20 p-6 rounded-2xl bg-blue-500/5 space-y-6">
                        <div>
                            ...
                            <textarea name="rows[0][uraian_peristiwa]" required rows="2" class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-blue-500/30 text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none resize-none" placeholder="Tuliskan uraian peristiwa yang terjadi..."></textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[11px] font-black text-blue-400 uppercase tracking-widest mb-3 ml-1">Waktu Kejadian</label>
                                <input type="text" name="rows[0][waktu_kejadian]" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-blue-500/30 text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="Contoh: 15 Agustus 2026">
                            </div>
                            <div>
                                <label class="block text-[11px] font-black text-blue-400 uppercase tracking-widest mb-3 ml-1">Tempat Kejadian</label>
                                <input type="text" name="rows[0][tempat_kejadian]" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-blue-500/30 text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="Contoh: Ruang Server">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="rows[0][skor_dampak]" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-rose-500/30 text-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all outline-none cursor-pointer">
                            ...
                            <div>
                                ...
                                <input type="text" name="rows[0][pemicu_peristiwa]" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-blue-500/30 text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="Penyebab memicunya peristiwa ini...">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="pt-10 flex space-x-4 border-t border-slate-800 mt-12">
            <button type="submit" class="px-10 py-4 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-2xl transition-all shadow-xl shadow-blue-500/20 active:scale-95 flex items-center gap-3">
                ...
                <span>Simpan Semua Peristiwa</span>
            </button>
        </div>
    </form>
</div>
...
<script>
    ...
    function updatePemantauanInfo(element) {
        const row = element.closest('.row-item');
        const previewDiv = row.querySelector('.preview-container');
        const selectedOption = element.options[element.selectedIndex];
        
        if (element.value) {
            row.querySelector('.ro_kode').innerText = selectedOption.dataset.kode || '';
            row.querySelector('.ro_kodepenyebab').innerText = selectedOption.dataset.kodepenyebab || '';
