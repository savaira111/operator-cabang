...
@section('page_title', 'Pemantauan Peristiwa Risiko')
...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
    <div class="relative group w-full max-w-md">
        ...
        <input type="text" id="searchInput" onkeyup="filterTable()" class="w-full bg-[#111827] border border-slate-800 text-white text-sm rounded-2xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 block pl-14 p-4 transition-all" placeholder="Cari data peristiwa...">
    </div>

    <a href="{{ route('pemantauan-peristiwa.create') }}" class="px-6 py-3 bg-blue-500 text-white font-bold rounded-2xl flex items-center hover:bg-blue-600 transition-all duration-300 shadow-xl shadow-blue-500/30 active:scale-95 shrink-0">
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-800/40 border-b border-slate-800/60">
                    ...
                    <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap">Uraian Peristiwa</th>
                    <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap">Tempat Kejadian</th>
                    <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap">Skor Dampak</th>
            ...
            <tbody class="divide-y divide-slate-800/40">
                @forelse($peristiwas as $peristiwa)
                <tr class="hover:bg-slate-800/30 transition-all group">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
                            {{ $peristiwa->pemantauanKegiatan->rencanaTindak->resiko->kode ?? '-' }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="font-bold text-white tracking-tight text-sm block min-w-[200px]">
                            {{ $peristiwa->pemantauanKegiatan->rencanaTindak->resiko->pernyataan_risiko ?? '-' }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="font-bold text-slate-300 tracking-tight text-sm block min-w-[200px]">
                            {{ Str::limit($peristiwa->uraian_peristiwa, 50) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-sm text-slate-400 font-bold uppercase tracking-tight whitespace-nowrap">
                        {{ $peristiwa->tempat_kejadian ?? '-' }}
                    </td>
                    <td class="px-6 py-4 text-sm font-bold tracking-tight whitespace-nowrap">
                        <span class="px-3 py-1 rounded-full text-xs {{ $peristiwa->skor_dampak >= 4 ? 'bg-rose-500/20 text-rose-400' : ($peristiwa->skor_dampak == 3 ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400') }}">
                            {{ $peristiwa->skor_dampak }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end space-x-2 transition-all">
                            <a href="{{ route('pemantauan-peristiwa.show', $peristiwa) }}" class="p-2.5 text-slate-500 hover:text-indigo-400 hover:bg-indigo-400/10 rounded-2xl transition-all">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </a>
                            <a href="{{ route('pemantauan-peristiwa.edit', $peristiwa) }}" class="p-2.5 text-slate-500 hover:text-blue-400 hover:bg-blue-400/10 rounded-2xl transition-all">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('pemantauan-peristiwa.destroy', $peristiwa) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-8 py-10 text-center text-slate-500 text-sm italic">Belum ada data pemantauan peristiwa.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
