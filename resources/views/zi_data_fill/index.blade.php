...
@section('title', 'Mengisi Data ZI')
@section('page_title', 'Mengisi Data ZI')

@section('content')
<div class="mb-8">
    <h3 class="text-3xl font-black text-white tracking-tighter uppercase">Mengisi Data ZI</h3>
    <p class="text-slate-500 text-sm mt-1 tracking-tight">Unggah data dukung rencana aksi sesuai dengan periode pelaksanaan.</p>
</div>
                                        ...
                                        <td class="px-6 py-6 border-r border-slate-800/60 w-56">
                                            <div class="flex items-center gap-1.5">
                                                @foreach($firstRk->files as $file)
                                                    <a href="{{ Storage::url($file->file_path) }}" target="_blank" class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-800 border border-slate-700 text-indigo-400 hover:bg-indigo-500 hover:text-white transition-all shadow-sm" title="{{ $file->period }}">
                                                        <span class="text-[9px] font-black">{{ $file->period }}</span>
                                                    </a>
                                                @endforeach
                                                @if($firstRk->files->isEmpty())
                                                    <span class="text-[10px] text-slate-700 italic">Belum ada file</span>
                                                @endif
                                            </div>
                                        </td>
