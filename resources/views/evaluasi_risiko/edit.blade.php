...
@section('title', 'Edit Evaluasi Risiko')
@section('page_title', 'Edit Evaluasi')

@section('content')
<div class="w-full bg-[#111827] border border-slate-800 rounded-[2.5rem] p-10 shadow-2xl">
    <div class="mb-10 flex items-start justify-between">
        <div>
            <h3 class="text-2xl font-black text-white tracking-tight text-emerald-400">Edit Hasil Evaluasi / Komentar</h3>
            <p class="text-slate-500 text-sm mt-1">Perbarui hasil evaluasi efektivitas pengendalian.</p>
        </div>
    ...
    <form action="{{ route('evaluasi-risiko.update', $evaluasiRisiko) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="bg-slate-800/30 p-8 rounded-[2rem] border border-slate-700/50">
                ...
                <select name="resiko_id" id="kode_select" required class="w-full px-5 py-4 bg-[#111827] rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none cursor-pointer" onchange="updateFields()">
                    @foreach($resikos as $r)
                        ...
                        @endphp
                        <option value="{{ $r->id }}" {{ $evaluasiRisiko->resiko_id == $r->id ? 'selected' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-6"><div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="pemilik_risiko" value="{{ $evaluasiRisiko->pemilik_risiko }}" required class="w-full px-5 py-4 bg-slate-800/50 rounded-2xl border border-slate-700 text-white focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none" placeholder="Masukkan nama unit / pegawai...">
                    </div>
                </div>
            </div>
